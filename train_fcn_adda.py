...
def main(output, dataset, datadir, lr, momentum, snapshot, downscale, cls_weights, gpu, 
    ...
    if weights_shared:
        ...
        net_src.eval()
    #net_src = torch.nn.DataParallel(net_src)
    ...
    W = net.transform
    loader = AddaDataLoader(W, dataset, datadir, downscale, 
    ...
    while iteration < max_iter:
        ...
        for im_s, im_t, label_s, label_t in loader:
            ...
            #print(label_s.size())
            #print(label_t.size())
            if (im_s.size()[3] < im_t.size()[3]):
                im_t = im_t[:,:,:,0:im_s.size()[3]]   
                label_t = label_t[:,:,0:im_s.size()[3]]   
            else:
                im_s = im_s[:,:,:,0:im_t.size()[3]]   
                label_s = label_s[:,:,0:im_t.size()[2]]   
            ...
            dis_label_concat = make_variable(
                    torch.cat(
                        ...
                        torch.zeros(batch_t,h,w).long()),2
                        ), requires_grad=False)
            #print(dis_score_t.size())
            #print(dis_score_s.size())
            # compute loss for discriminator
            #dis_pred_concat = dis_pred_concat[0]
            #dis_label_concat = dis_label_concat.view(dis_label_concat.size(0), -1)
            #dis_label_concat = dis_label_concat.unsqueeze(0)
            #print(dis_pred_concat.shape)
            #print(dis_label_concat.shape)
