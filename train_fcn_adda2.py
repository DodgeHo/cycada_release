...
def forward_pass(net, discriminator, im, requires_grad=False, discrim_feat=False):
    ...
    if not requires_grad:
        score = Variable(score.data, requires_grad=False).half()
...
def discriminator_loss(score, target_val, lsgan=False):
    if lsgan:
        ...
        _,_,h,w = score.size()
        target_val_vec = Variable(target_val * torch.ones(1,h,w),requires_grad=False).long().cuda().half()
    ...
    while iteration < max_iter:
        ...
        for im_s, im_t, label_s, label_t in loader:
            ...
            if discrim_feat:
                ...
                score_s = Variable(score_s.data, requires_grad=False).half()
                f_s = Variable(feat_s.data, requires_grad=False).half()
            else:
                score_s = Variable(net_src(im_s).data, requires_grad=False).half()
                f_s = score_s
            dis_score_s = discriminator(f_s)
            
            if discrim_feat:
                score_t, feat_t = net(im_t)
                score_t = Variable(score_t.data, requires_grad=False).half()
                f_t = Variable(feat_t.data, requires_grad=False).half()
            else:
                score_t = Variable(net(im_t).data, requires_grad=False).half()
                f_t = score_t
            dis_score_t = discriminator(f_t)
            print("dis_score:")
            print(dis_score_s.shape)
            print(dis_score_t.shape)
            
            dis_pred_concat = torch.cat((dis_score_s, dis_score_t),3)
